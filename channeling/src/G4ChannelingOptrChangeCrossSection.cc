...
G4ChannelingOptrChangeCrossSection::G4ChannelingOptrChangeCrossSection(G4String particleName, G4String name)
:G4VBiasingOperator(name),
fChannelingID(G4PhysicsModelCatalog::GetModelID("model_channeling")), fSetup(true) 
{
...
G4ChannelingOptrChangeCrossSection::~G4ChannelingOptrChangeCrossSection()
{
	for (std::map< const G4BiasingProcessInterface*, G4BOptnChangeCrossSection* >::iterator
		 it = fChangeCrossSectionOperations.begin(); it != fChangeCrossSectionOperations.end(); it++ )
		 	delete (*it).second;
}
...
G4VBiasingOperation*
G4ChannelingOptrChangeCrossSection::ProposeOccurenceBiasingOperation(const G4Track* track, const G4BiasingProcessInterface* callingProcess)
{
	...
	G4ChannelingTrackData* trackdata = (G4ChannelingTrackData*)(track->GetAuxiliaryTrackInformation(fChannelingID));
