...
private:
    ...
    //G4GenericMessenger* fGenMessenger;
    ...
    //DetectorConstruction* GetDetector();
private:
...
public:
    ...
    G4double GetCriticalAngle2(const G4Track& aTrack){ return std::sqrt(GetMatData(aTrack)->GetPot()->GetMaxMin()/GetPre(aTrack)->GetKineticEnergy());}
    ...
    G4double GetOscillationPeriod2(const G4Track& aTrack){
        return (CLHEP::pi * GetMatData(aTrack)->GetPot()->GetIntSp(0)
                / GetCriticalAngle2(aTrack));}

    //G4double GetRealCriticalAngle(G4double energy) { return std::sqrt(2.0*GetMatData(aTrack)->GetPot()->GetMaxMin()/GetPre(aTrack)->GetTotalEnergy());}
   ...
   G4double GetCriticalEnergy(const G4Track& aTrack){
	...
	G4double e2 = 1.4399764*(MeV*CLHEP::fermi); // MeV*fm
...
private:
    ...
    G4double chan_step;
    G4int enable_rechanneling;
    G4double density_limit;
    G4int v11_algo;
    G4int org_step_size;

    G4bool use_step_size;
    G4double step_size_value;
    G4double mfp_value;
