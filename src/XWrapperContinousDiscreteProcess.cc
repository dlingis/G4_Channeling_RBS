...
XWrapperContinuousDiscreteProcess::
XWrapperContinuousDiscreteProcess(const G4String& aName, G4VContinuousDiscreteProcess* toRegister)
:G4VContinuousDiscreteProcess(aName){
...
XWrapperContinuousDiscreteProcess::
XWrapperContinuousDiscreteProcess(const G4String& aName, G4ProcessType aProcessType)
...
void XWrapperContinuousDiscreteProcess::
RegisterProcess(G4VContinuousDiscreteProcess* toRegister, G4int flag, G4int region){
	fRegisteredProcess = toRegister;
	theProcessType = fRegisteredProcess->GetProcessType();
	theProcessSubType = fRegisteredProcess->GetProcessSubType();
	enableAtRestDoIt = fRegisteredProcess->isAtRestDoItIsEnabled();
...
G4int XWrapperContinuousDiscreteProcess::ItHasToWork(const G4Track& aTrack){
		...
		ExExChParticleUserInfo* chanInfo = (ExExChParticleUserInfo*) aTrack.GetUserInformation();
...
GetMeanFreePath(const G4Track& aTrack, G4double previousStepSize, G4ForceCondition* condition){
	if (ItHasToWork(aTrack) == 2) {
		return fRegisteredProcess->PostStepGetPhysicalInteractionLength(aTrack, previousStepSize, condition);
	}
	return DBL_MAX;
}
...
G4double XWrapperContinuousDiscreteProcess::
PostStepGetPhysicalInteractionLength (const G4Track &aTrack, G4double previousStepSize, G4ForceCondition *condition){
...
G4VParticleChange* XWrapperContinuousDiscreteProcess::
AlongStepDoIt(const G4Track& aTrack, const G4Step& aStep ){
...
G4double XWrapperContinuousDiscreteProcess::
GetContinuousStepLimit(const G4Track&, G4double, G4double, G4double& ){
	return DBL_MAX;
}
