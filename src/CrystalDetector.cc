...
void CrystalDetector::Initialize(G4HCofThisEvent*HCE) {
	fHitsCollection = new CrystalDetectorHitsCollection(SensitiveDetectorName, collectionName[0]);
	if (fHCID < 0) {
...
G4bool CrystalDetector::ProcessHits(G4Step*aStep, G4TouchableHistory*){
	...
	G4ChannelingTrackData* trackdata = (G4ChannelingTrackData*)(aStep->GetTrack()->GetAuxiliaryTrackInformation(fChannelingID));
	...
	CrystalDetectorHit* aHit = new CrystalDetectorHit(copyNo);
	aHit->SetLayerID(copyNo);
	
	G4ThreeVector pos = trackdata->GetPosCh();
	
	if (aStep->GetTrack()->GetKineticEnergy() /MeV > 0.1) {
		...
		aHit->SetChPos(pos);
		// only two elements supported for now. Shifted elements.
		if (set_name1)
			aHit->SetNuD_a(fNucleiDensity_b->GetEC(pos));
		else
			aHit->SetNuD_a(1.);
		if (set_name2) 
			aHit->SetNuD_b(fNucleiDensity_c->GetEC(pos));
		else
			aHit->SetNuD_b(1.);
		if (set_name3) 
			aHit->SetNuD_c(fNucleiDensity_a->GetEC(pos));
		else
			aHit->SetNuD_c(1.);
